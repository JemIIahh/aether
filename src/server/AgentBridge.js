...
export class AgentBridge {
  ...
  _execAgent(message) {
    return new Promise((resolve, reject) => {
      ...
      const cmd = `aetherist agent --session-id "${this.sessionId}" --message '${escaped}' --json --timeout 30`;
  ...
  getPhasePrompt(phase, context) {
    switch (phase) {
      case 'welcome':
        return `**Phase: WELCOME** — Players are joining! Greet each player BY NAME dramatically. Introduce yourself as the The Aetherist. Tease what's coming. Use send_chat_message and announce tools. If you have enough players, start building an arena (clear_world, then spawn entities or load_template).`;
