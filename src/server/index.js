...
import { ZeroGChainInterface } from './blockchain/ZeroGChainInterface.js';
import { OG_RPC_URL } from '../shared/chains.js';
...
const chain = isRealChain
  ? new ZeroGChainInterface({
      rpcUrl: process.env.OG_RPC_URL || OG_RPC_URL,
...
const BRIBE_OPTIONS = {
  spawn_obstacles: {
    ...
    cost: 50, cost0G: '0.002', costWei: '2000000000000000'
  },
  lava_floor: {
    label: 'Turn Floor to Lava', description: 'Floor becomes deadly lava',
    cost: 100, cost0G: '0.005', costWei: '5000000000000000'
  },
  random_spell: {
    label: 'Cast Random Spell', description: 'Random spell on all players',
    cost: 30, cost0G: '0.001', costWei: '1000000000000000'
  },
  move_goal: {
    label: 'Move the Goal', description: 'Relocate the goal (reach games)',
    cost: 75, cost0G: '0.003', costWei: '3000000000000000'
  },
  extra_time: {
    label: 'Extra Time (+15s)', description: 'Add 15 seconds to the clock',
    cost: 40, cost0G: '0.002', costWei: '2000000000000000'
  },
  custom: {
    label: 'Custom Request', description: 'Free-text request for The Aetherist',
    cost: 200, cost0G: '0.01', costWei: '10000000000000000'
  }
};
  ...
  switch (bribeType) {
    ...
    case 'move_goal': {
      if (arena.currentMiniGame?.isActive && arena.currentMiniGame.type === 'reach' && arena.currentMiniGame.targetEntityId) {
        ...
        broadcast('announcement', ws.announce('The Aetherist notes your bribe... the goal will shift next game!', 'agent', 5000));
      }
      break;
    }
    ...
    case 'extra_time': {
      if (arena.currentMiniGame?.isActive) {
        ...
        broadcast('announcement', ws.announce('The Aetherist pockets the bribe... extra time next game!', 'agent', 5000));
      }
      break;
    }
...
// Default arena (aether) passes through automatically — requireArenaKey skips it.
...
gameRouter.post('/bribe', requireAuth, async (req, res) => {
  ...
  const amount = option.cost0G;
  const costLabel = `${option.cost0G} 0G`;
  ...
  arena.broadcastToRoom('announcement', ws.announce(
    `${name} bribed the Aetherist (${option.label}) for ${costLabel}!`, 'player', 8000
  ));
...
gameRouter.post('/bribe/:id/honor', async (req, res) => {
  ...
  const announcement = ws.announce(
    `The Aetherist honors ${name}'s bribe!${response ? ` "${response}"` : ''}`, 'agent', 8000
  );
  arena.broadcastToRoom('announcement', announcement);
...
gameRouter.post('/tokens/faucet', requireAuth, async (req, res) => {
  if (isRealChain) {
    return res.status(400).json({ error: 'Faucet not available on mainnet. Send 0G to your wallet address.' });
  }
