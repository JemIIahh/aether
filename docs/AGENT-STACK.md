# Agent Stack: Aetherist runtime + Game World

## Overview

We use **Aetherist runtime** as the agent runtime for the The Aetherist and AI players. Aetherist runtime provides:
...
The game world (Three.js + Colyseus) exposes an HTTP API that Aetherist runtime agents call via custom skills.
...
```
~/.aetherist/workspace/skills/game-world/
...
2. Aetherist runtime routes to The Aetherist agent
...
2. Game server sends WebSocket event to Aetherist runtime
...
```json
{
  "agents": {
    "chaos-magician": {
      "workspace": "~/.aetherist/workspace/chaos-magician",
      "model": "claude-sonnet-4-5"
    },
    "explorer": {
      "workspace": "~/.aetherist/workspace/explorer",
      "model": "claude-haiku"
    },
    "chaotic": {
      "workspace": "~/.aetherist/workspace/chaotic",
      "model": "claude-haiku"
    }
  },
  "bindings": [
...
AI players don't directly talk to The Aetherist - they just play the game, and the Aetherist observes and reacts.
...
```
~/.aetherist/workspace/chaos-magician/memory/
...
### 1. Configure Aetherist runtime for game agent

```bash
# Create chaos magician workspace
mkdir -p ~/.aetherist/workspace/chaos-magician
cd ~/.aetherist/workspace/chaos-magician
...
```bash
mkdir -p ~/.aetherist/workspace/skills/game-world
...
### 4. Start Aetherist runtime gateway

```bash
aetherist gateway --port 18789
```
...
## Why Aetherist runtime Over Claude Agent SDK

| Requirement | Aetherist runtime | Claude Agent SDK |
|-------------|----------|------------------|
...
**Verdict**: Aetherist runtime is purpose-built for persistent, interactive AI agents with personality. Claude Agent SDK is better for code-focused tasks. For the The Aetherist character, Aetherist runtime is the right choice.
