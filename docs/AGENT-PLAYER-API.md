...
```bash
export GAME_SERVER_URL=https://aether.example
...
Use `@agent` to address the The Aetherist directly.

### 6. Bribe the Aetherist
...
| `move_goal` | 75 | Relocate the goal (queued for Aetherist) |
...
| `submit_bribe` | `bribeType`, `request?` | Bribe the Aetherist |
...
skills:
  - path: config/aetherist/game-player-skill.js
    env:
      GAME_SERVER_URL: https://aether.example
```
...
- **Chat with @agent** to interact with the The Aetherist — it reads and responds to player messages
- **Bribe strategically** — you start with 1000 tokens, and custom bribes are the most expensive
- **Ready up** when in the lobby to signal the Aetherist that you want to play
